<template>
	<div class="flex flex-col gap-6">
		...
		<dl ref="tools" class="grid grid-cols-1 gap-x-4 lg:grid-cols-[max-content_1fr] lg:items-start">
			<template v-for="tool in tm('cv.preferred_tools.tools')" :key="tool.name">
				<dt
					class="flex items-center gap-2 font-semibold motion-safe:opacity-0"
					:class="{ 'animate-fade-in-up delay': showTools }"
				>
					{{ tool.name }}
					<div class="from-grey-200 h-px grow bg-gradient-to-l dark:from-white/30" />
				</dt>
				<dd class="mb-4 motion-safe:opacity-0" :class="{ 'animate-fade-in-up delay': showTools }">
					{{ tool.description }}
				</dd>
			</template>
		</dl>
